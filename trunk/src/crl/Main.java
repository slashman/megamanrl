...
import crl.data.MegaManMonsters;
...
public class Main {
	...
	private static void init(){
		if (createNew){
			System.out.println("MegamanRL "+Game.getVersion());
			System.out.println("Slash ~ 2008");
	...
	private static void readConfiguration(){
	    ...
	    try {
	    	configuration.load(new FileInputStream("mmrl.cfg"));
	    } catch (IOException e) {
	    	System.out.println("Error loading configuration file, please confirm existence of mmrl.cfg");
	    	System.exit(-1);
	    }
	...
	private static void	title() {
		...
		case 2:
			...
			break;
		case 3:
			System.out.println("MegamanRL v"+Game.getVersion()+", clean Exit");
	...
	private static void initializeUI(Object si){
		...
		Action fire = new UseWeapon();
		Action dive = new Dive();
		Action turn = new TurnAround();
		
		//Action fusion = new FusionSpirits();

		UserAction[] userActions = new UserAction[] {
		        new UserAction(fire, CharKey.x),
		        new UserAction(jump, CharKey.z),
		        new UserAction(equip, CharKey.s),
		        new UserAction(get, CharKey.COMMA),
		        new UserAction(use, CharKey.d),
		        ...
		        new UserAction(dive, CharKey.D),
		        new UserAction(target, CharKey.f),
		        //new UserAction(fusion, CharKey.F),
		...
		UserCommand[] userCommands = new UserCommand[]{
			...
			new UserCommand(CommandListener.STAGESELECT, CharKey.W),
		...
		case JCURSES_CONSOLE:
			...
			((ConsoleUISelector)uiSelector).init((ConsoleSystemInterface)si, userActions, walkAction, (ConsoleUserInterface)ui);
			break;
		case SWING_CONSOLE:
	...
	private static void initializeMonsters() throws CRLException{
		...
		MonsterFactory.getFactory().init(MegaManMonsters.getMonsterDefinitions("data/monsters.csv","data/monsters.xml"));
	}
    ...
    public static void crash(String message, Throwable exception){
    	System.out.println("MegamanRL "+Game.getVersion()+": Error");
        System.out.println("");
