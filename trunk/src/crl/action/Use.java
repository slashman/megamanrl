...
public class Use extends Action{
	...
	public void execute(){
		...
		if (def.getAction() != null && !def.getAction().equals("")){
			Action actionObject = null;
			
			try {
				actionObject = (Action)Class.forName(def.getAction()).newInstance();
		...
		if (def.getID().equals("SOUL_RECALL")){
			...
			aPlayer.landOn(Position.add(aPlayer.getLevel().getExitFor("FOREST0"), new Position(-1,0,0)));
