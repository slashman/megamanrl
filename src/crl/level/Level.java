...
public class Level implements FOVMap, Serializable{
	...
	public Actor getActorAt(Position x){
		ArrayList<Actor> actors = dispatcher.getActors();
...
/*
	...
	public void monsterExplosion(Monster m, Position p){
		SFXManager.play("wav/lazrshot.wav");
		addEffect(EffectFactory.getSingleton().createLocatedEffect(p, "SFX_BOMB"));
		if (m.damagesEnemies()){
			ArrayList<Actor> monsters = new ArrayList<Actor>(getActors());
			for (int i = 0; i < monsters.size(); i++){
				Actor a = monsters.get(i);
				if (a instanceof Monster){
					Monster t = (Monster)a;
					if (Position.distance(t.getPosition(), p) < 2){
						t.damage(m.getAttack());
					}
				}
			}
		} else {
			if (Position.distance(getPlayer().getPosition(), p) < 2){
				getPlayer().damage(m, m.getAttack());
			}
		}
	}
