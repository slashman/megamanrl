	...
	public void execute(){
		...
		if (weapon == null || weapon.getDefinition().getWeaponCategory().equals(ItemDefinition.CAT_UNARMED)) {
			...
			if (targetMonster != null && targetMonster.getStandingHeight() == player.getStandingHeight()){
				aLevel.addMessage("You "+attackDescription+" the "+targetMonster.getDescription()+"!");
				targetMonster.damageWithWeapon(punchDamage);
				if (push != 0)
	...
	private void pushMonster(Monster targetMonster, Level aLevel, int spaces){
		...
		for (int i = 0; i < spaces; i++){
			...
			if (!fly.isSolid()){
				...
				aLevel.addMessage("You smash the "+targetMonster.getDescription()+" against the "+fly.getDescription()+"!");
				targetMonster.damageWithWeapon(2);
			}
			runner.add(varP);
		}
